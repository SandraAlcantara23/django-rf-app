...
META_PATH = MODEL_DIR / "rf_meta.joblib"   # guardamos columnas esperadas
...
def load_model():
    ...
    meta = joblib.load(META_PATH)  # {'features': [...], 'target': '...'}
    return model, meta
...
def ensure_columns(df: pd.DataFrame, features: List[str]) -> pd.DataFrame:
    # Orden y subset a lo que espera el modelo
    missing = [c for c in features if c not in df.columns]
    if missing:
        raise ValueError(f"Faltan columnas en CSV: {missing}")
    return df[features].copy()

def predict_df(df: pd.DataFrame) -> Tuple[pd.DataFrame, List[float]]:
